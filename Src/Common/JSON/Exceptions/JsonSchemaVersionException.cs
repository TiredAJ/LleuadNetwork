...
public class JsonSchemaVersionException : Exception
{
    ...
    static private string GetDebugMessage(string _SchemaName, int _ExpectedVer, int? _CurrentVer)
        => $"{_SchemaName} schema failed version validation. Expected version: [{_ExpectedVer}], File version: [" +
           _CurrentVer is null ? "null" : _CurrentVer + "].";
